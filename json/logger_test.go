...
func utilTestLevel(t *testing.T, out string, level mlog.Level) {
	if inc := strings.Contains(out, `"level":"fatal"`); inc != (level >= mlog.Fatal) {
	...
	if inc := strings.Contains(out, `"level":"panic"`); inc != (level >= mlog.Panic) {
	...
	if inc := strings.Contains(out, `"level":"error"`); inc != (level >= mlog.Error) {
	...
	if inc := strings.Contains(out, `"level":"warning"`); inc != (level >= mlog.Warning) {
	...
	if inc := strings.Contains(out, `"level":"info"`); inc != (level >= mlog.Info) {
	...
	if inc := strings.Contains(out, `"level":"verbose"`); inc != (level >= mlog.Verbose) {
